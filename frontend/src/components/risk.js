export function init(container) {
  container.innerHTML = `
    ...
    <div class="calc-grid">
      <div class="card">
        <h3>VaR & CTE Calculator</h3>
        <div class="field"><label>Losses (comma or space-separated, or JSON array)</label>
          <textarea id="risk-losses" rows="8">0.01, 0.02, 0.015, 0.03, 0.025, 0.04, 0.035, 0.05, 0.045, 0.06, 0.055, 0.07, 0.065, 0.08, 0.075, 0.09, 0.085, 0.1, 0.095, 0.12</textarea>
        </div>
        <button onclick="window._riskCompute()">Compute Metrics</button>
    ...
    try {
      ...
      document.getElementById('risk-result').innerHTML =
        `Count: ${losses.length}<br>
         ...
         Min: ${report.min.toFixed(6)} | Max: ${report.max.toFixed(6)}<br>
         <hr>
         ...
         CTE(99%): <strong>${report.cte99.toFixed(6)}</strong><br>
         <hr>
